...
const getArticleItemCache = async (ctx, articleUrl) => {
    ...
    if (title) {
        ...
        const pubDate = await ctx.cache.get(articleUrl + pubDateSuffix);
        const author = await ctx.cache.get(articleUrl + authorSuffix);
        return { title, description, pubDate, author, link: articleUrl };
    } else {
        return null;
    }
};
...
const setArticleItemCache = (ctx, articleUrl, item) => {
    ...
    ctx.cache.set(articleUrl + pubDateSuffix, item.pubDate);
...
const getArticleItem = async (ctx, page, articleUrl) => {
    ...
    if (item) {
        ...
        try {
            ...
            const pubDate = await page.evaluate(
                () => document.querySelector('div.ArticleHeader_date').textContent
            );
            const author = await page.evaluate(
                () => document.querySelector('div.BylineBar_byline').textContent
            );
            item = { title, description, pubDate, author, link: articleUrl };
